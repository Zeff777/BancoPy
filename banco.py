...
def criar_conta() -> None:
    ...
    cliente: Cliente = Cliente(nome, email, cpf, data_nascimento=data_nascimento)
...
def efetuar_transferencia() -> None:
    if len(contas) > 0:
        ...
        if conta_origem:
            ...
            if conta_destino:
                valor: float = float(input('Inform o valor da transfência: '))
                conta_origem.transferir(conta_destino, valor)

            else:
                print(f'Conta destino com número: {numero_destino}, não foi encontrada.')

        else:
            print(f'Conta com número: {numero_origem}, não foi encontrada')
